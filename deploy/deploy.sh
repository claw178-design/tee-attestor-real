...
#
# Usage:
#   ECLOUD_PRIVATE_KEY=0x... ./deploy.sh            # deploy new
#   ECLOUD_PRIVATE_KEY=0x... APP_ID=0x... ./deploy.sh  # upgrade existing
...
# Required signing key for ecloud CLI
...
if [ -n "$APP_ID" ]; then
  echo "[upgrade] app=$APP_ID image=$IMAGE_REF env=$ENVIRON log=$LOG_VIS"
  # shellcheck disable=SC2068
  printf 'N\n' | ecloud compute app upgrade "$APP_ID" ${ECLOUD_ARGS[@]}
else
  echo "[deploy] name=$APP_NAME image=$IMAGE_REF env=$ENVIRON log=$LOG_VIS"
  # shellcheck disable=SC2068
